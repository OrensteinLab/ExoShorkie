...
SRR_LIST=("SRR27411956" "SRR27411955")

TMP_DIR="tmp"
STAR_INDEX_DIR="${TMP_DIR}/STAR_index"
OUTPUT_DIR="${TMP_DIR}/output"
GENOME_FA="HPRT1R_reference.fa"


# Set to 1 if you want to delete FastQC reports to save space
DELETE_FASTQC=0
...
for SRR in "${SRR_LIST[@]}"; do
    ...
    # === CLEANUP PART 1: remove FASTQs (raw + trimmed) ===
    ...
    # Optional: delete FastQC outputs
...
# === CLEANUP PART 2: remove individual replicate BAMs + BAIs ===
...
bamCoverage -b "${TMP_DIR}/merged.bam" \
    -o "${OUTPUT_DIR}/HPRT1R_merged_fwd_cpm.bw" \
...
bamCoverage -b "${TMP_DIR}/merged.bam" \
    -o "${OUTPUT_DIR}/HPRT1R_merged_rev_cpm.bw" \
    --outFileFormat bigwig \
